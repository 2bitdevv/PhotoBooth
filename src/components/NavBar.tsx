    ...
    <>
      <header className="fixed top-0 z-30 mx-auto flex w-full justify-center px-4 pt-3">
        <nav className="relative flex w-fit items-center gap-2 rounded-full border border-white/70 bg-white/55 px-3 py-2 shadow-lg backdrop-blur-md">
          ...
          {isOpen && (
            <div className="absolute right-0 top-14 z-30 w-56 rounded-2xl border border-slate-200 bg-white p-3 shadow-lg md:hidden">
              {links.map((item) => (
                <Link
                  ...
                  className="block rounded-xl px-3 py-2 text-sm font-semibold text-slate-800 hover:bg-blue-50"
                >
                  {item.label}
                </Link>
              ))}
